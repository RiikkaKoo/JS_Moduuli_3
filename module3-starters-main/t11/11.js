...
t11.txt. Modify the program to open large image in a modal when <article>
is clicked. (6p)
kick yourself at this point if you used innerHTML to create the <article>
and its content.
...
picArray has two images for each item: medium and large. Medium is used in the
<img> inside the <article> and large is used in the <img> inside the <dialog>.
use showModal() and close() functions to show and hide <dialog>
the same time you are opening the modal, you should put the large image to the
<img> in the modal.
...
for (let a = 0; a < picArray.length; a++) {
  let artikkeli = document.createElement('article');
  artikkeli.classList.add('card');
  document.querySelector('section').appendChild(artikkeli).innerHTML =
      `<h2>${picArray[a]['title']}</h2>
       <figure>
            <img src=${picArray[a]['image']['medium']} alt=${picArray[a]['title']}>
            <figcaption>${picArray[a]['caption']}</figcaption>
       </figure>
       <p>${picArray[a]['description']}</p>`;
}

// Event funktio avaa dialogin ja hakee oikena kuvan klikatun artikkelin
// otsikon numeron perusteella:
function open_modal(event) {
  let dialogi = document.querySelector('dialog');
  // console.log(dialogi);
  dialogi.showModal();

  // Haetaan klikatun artikkelin tiedot:
  let valittu = event.currentTarget;
  // console.log(valittu);

  // Valitaan otskko ja pilkotaan se kahteen osaan, jotta saadaan numero käyttöön:
  let otsikko = valittu.querySelector('h2').innerHTML;
  let pilkottu = otsikko.split(' ');
  // console.log('pilkottu', pilkottu);
  let nro = pilkottu[1];
  // console.log('nro', nro);

  // Saadulla numeron avulla lisätään/muutetaan dialogin kuvaelementti:
  let d_kuva = document.querySelector('dialog img');
  d_kuva.src = `img/pic${nro}.jpg`;
  d_kuva.alt = `${otsikko}`;
}

// Dialogin span-elementtiin lisättävä sulkemisfunktio:
function close_modal(event) {
  let dialogi = document.querySelector('dialog');
  dialogi.close();
}

// Lisätään kaikkiin artikkeli-elementteihin dialogin avausfunktio:
const artikkelit = document.querySelectorAll('.card');
console.log(artikkelit);
for (let a of artikkelit) {
  a.addEventListener('click', open_modal);
}

// Lisätään sulkemisfunktio:
document.querySelector('dialog span').addEventListener('click', close_modal);